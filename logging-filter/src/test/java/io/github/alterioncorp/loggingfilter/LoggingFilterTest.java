...
public class LoggingFilterTest {
	...
	public void before() {
		...
		// stub getHeaderNames to return empty by default
		Mockito.when(httpServletRequest.getHeaderNames()).thenReturn(Collections.emptyEnumeration());
		Mockito.when(httpServletResponse.getHeaderNames()).thenReturn(Collections.emptyList());
	}
	...
	public void testInit_Loggers() throws Exception {
		...
		Mockito.verify(infoLogger).init(Mockito.any(ConfigurationDefaultImpl.class));
	}
	...
	@Test
	public void testDestroy_DeregisterWithJmx() throws Exception {
	...
	@Test
	public void testDestroy_BeforeInit() throws Exception {
	...
	public void testDoFilter_LoggerDisabled() throws Exception {
		...
		Mockito.verify(infoLogger, Mockito.never()).logRequest(Mockito.any());
		Mockito.verify(infoLogger, Mockito.never()).logResponse(Mockito.any());
	}

	@Test
	public void testDoFilter_RequestHeaders() throws Exception {
		...
		Mockito
			...
			.thenReturn(Collections.enumeration(Arrays.asList("v11")));
		Mockito
		...
		Map<String, List<String>> headers = infos.requestInfo.getHeaders();
		assertNotNull(headers);
		assertTrue(headers.containsKey("h1"));
		assertTrue(headers.containsKey("h2"));
		assertEquals(Arrays.asList("v11"), headers.get("h1"));
		assertEquals(Arrays.asList("v21"), headers.get("h2"));
	}
	...
	private Infos doFilter(String requestIP, String method, String uri, Map<String, String[]> params, int responseStatus) throws Exception {
		...
		Mockito.when(clientIpResolver.getClientIp(Mockito.any(), Mockito.any())).thenReturn(requestIP);
		...
		try {
			...
			Mockito.verify(infoLogger).logRequest(requestCaptor.capture());
			Mockito.verify(infoLogger).logResponse(responseCaptor.capture());
